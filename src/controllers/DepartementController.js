...
module.exports = {
  createdepartement: async (req, res) => {
    ...
    try {
      ...
      if (departement) {
        return res
          .status(409)
          .json({ message: " Departement already exists!" });
      }
  ...
  updatedepartement: async (req, res) => {
    ...
    try {
      ...
      if (Chef_Departement !== olddept.chef_departement) {
        const ens = await Enseignant.findByIdAndUpdate(
          { _id: Chef_Departement },
          { chef_departement: null },
          { new: true }
        );
        if (!ens) {
          res.status(404).json({
        ...
        if (olddept.chef_departement) {
          const oldens = await Enseignant.findByIdAndUpdate(
            { _id: olddept.Chef_Departement },
      ...
      const dept = await Departement.findByIdAndUpdate({ _id: _id }, req.body, {
        new: true,
      }).then(async (departement) => {
  ...
  archiverdepartement: async (req, res) => {
    ...
    try {
      const dept = await Departement.findOneAndUpdate(
        { nom: nom },
        { isActive: false },
        { new: true }
      ).then((departement) => {
