...
module.exports={
    createEvenement: async (req, res) => {
      console.log('seleyem',req.body)
        ...
        try {
         ...
         if (evenement){
          {return res.status(409).json({ message: " Evenement already exists!" });}
         }
          ...
          if (!savedEvenement) {return res.status(401).json({ message: " SQL ERROR!" });}
    ...
    accepterEvenement:async (req, res) => {
        console.log('seleyem',req.body)
          ...
          try {
            const evenement    = await Evenement.findOneAndUpdate({ _id: id }, {isAccepted:true}, {new:true}
              
            ).then((evenement)=>{console.log("seleeeeyem",evenement);
            if (!evenement) {
              res.status(500).json({
    ...
    updateEvenement: async (req, res) => {
        console.log('seleyem',req.body)
          ...
          try {
                ...
                const evenement = await Evenement.findByIdAndUpdate({_id:id}, req.body, {new:true}
            ).then(async(evenement)=>{console.log("seleeeeyem",evenement);
            if (!evenement) {
                res.status(500).json({
   ...
   archiverEvenement: async (req, res) => {
          console.log('seleyem',req.body)
            ...
            try {
              const evenement = await Evenement.findOneAndUpdate({ _id: id }, {isActive:false}, {new:true}
    ...
    activerEvenement:async (req, res) => {
      console.log('seleyem',req.body)
        ...
        try {
          const evenement = await Evenement.findOneAndUpdate({ _id: id }, {isActive:true}, {new:true}
