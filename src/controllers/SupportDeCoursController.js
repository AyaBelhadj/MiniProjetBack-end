...
module.exports = {
    createCours: async (req, res) => {
        ...
        try {
            ...
            if (!savedCours) { return res.status(401).json({ message: "Mongo ERROR!" }); }
    ...
    uploadFile : async (req, res) => {
      ...
      try {
        ...
        await pdf.save();
        const cours = await Cours.findByIdAndUpdate({ _id: idCours },{fichier :pdf._id }, {new:true}
          )
          if(!cours){
    ...
    updateCours: async (req, res) => {
        ...
        try {
            ...
            const cours = await Cours.findByIdAndUpdate({ _id: id }, req.body, { new: true }
            ).then(async (cours) => {
    ...
    getCoursByNameOrID: async (req, res) => {
        ...
        try {
            ...
            let cours;
            if (nom) {
                cours = await Cours.findOne({ nom: nom }).populate(["id_matiere","fichier"])
            } else {
                cours = await Cours.findOne({ _id: id }).populate(["id_matiere","fichier"])
            }
    ...
    getAllCours: async (req, res) => {
        try {
            const cours = await Cours.find({ fichier: { $ne: null } }).populate(["id_matiere","fichier"])
            .then((cours) => {
                console.log("seleeeeyem", cours);
                if (!cours) {
                    res.status(404).json({
...
if (!matiere){
  res.status(404).json({
    message: "matiere not found for the given subject",
      ...
      try {
          const cours = await Cours.findOne({ id_matiere: matiere._id }).populate(["id_matiere","fichier"]);
