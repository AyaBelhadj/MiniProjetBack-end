...
module.exports = {
  createFiliere: async (req, res) => {
    ...
    try {
      ...
      const filiere = await Filiere.findOne({
        ...
        niveau: { $regex: new RegExp(`^${niveau}$`, "i") },
      });
  ...
  updateFiliere: async (req, res) => {
    ...
    try {
      const filiere = await Filiere.findOneAndUpdate({ _id: id }, req.body, {
        new: true,
      }).then(async (filiere) => {
        console.log("seleeeeyem", filiere);
        if (!filiere) {
          res.status(500).json({
  ...
  archiverFiliere: async (req, res) => {
    ...
    try {
      const filiere = await Filiere.findOneAndUpdate(
        { _id: id },
        { isActive: false },
        { new: true }
      ).then((filiere) => {
  ...
  getFiliereByDep: async (req, res) => {
    ...
    try {
      const filiere = await Filiere.find({ id_departement: id });
  ...
  getallFilieres: async (req, res) => {
    try {
      const filiere = await Filiere.find({ isActive: true }).then((filiere) => {
        console.log("seleeeeyem", filiere);
        if (!filiere) {
          res.status(404).json({
            message: "filiere not found ",
