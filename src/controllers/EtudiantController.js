...
module.exports={
    createEtudiant: async (req, res) => {
      console.log('seleyem',req.body)
        ...
        try {
         ...
         if (!savedetudiant) throw Error("Something went wrong saving the etudiant");
    ...
    updateEtudiant: async (req, res) => {
        const { nom,prenom,adresse,dateNaiss,numTel,id_groupe,numInscription} = req.body;

    
        if (!nom ||!prenom||! adresse || !dateNaiss||! numTel||!numInscription/*!id_groupe||*/) {
          return res.status(400).json({ message: "Please enter all fields" });
        }
      
          try {
            const etud = await Etudiant.findOneAndUpdate({ numInscription: numInscription }, req.body, {new:true,projection: { password: 0 }})
            console.log(etud);
            if (!etud){
              res.status(404).json({
     ...
     archiverEtudiant: async (req, res) => {
          console.log('seleyem',req.body)
            ...
            try {
              const etud = await Etudiant.findOneAndUpdate({ numInscription: numInscription }, {isActive:false}, {new:true}
              ).then((etudiant)=>{console.log("seleeeeyem",etudiant);
              if (!etudiant) {
                res.status(500).json({
          ...
          activerEtudiant: async (req, res) => {
            console.log('seleyem',req.body)
              ...
              try {
                const etud = await Etudiant.findOneAndUpdate({ numInscription: numInscription }, {isActive:true}, {new:true}
                ).then((etudiant)=>{console.log("seleeeeyem",etudiant);
                if (!etudiant) {
                  res.status(500).json({
            ...
            console.log('seleyem',req.query)
              ...
              try {
                ...
                if(email){
                  ...
                  etud = await Etudiant.findOne({ numInscription: numInscription },"-password")
                }
  ...
  getallEtudiant: async (req, res) => {
              ...
              try {
                const etud = await Etudiant.find({isActive : true},"-password"
                ).then((etudiant)=>{console.log("seleeeeyem",etudiant);
                if (!etudiant) {
                  res.status(404).json({
                    message: "Etudiant not found ",
